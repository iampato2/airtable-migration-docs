# Detailed Airtable to Database Migration Mapping

## Overview

This document provides an exhaustive, field-by-field mapping between each Airtable table and the corresponding target database tables (PostgreSQL/MongoDB). Each Airtable column is explicitly mapped to show exactly how data will be transformed and stored in the new system.
...
| `tttt` | `debug_data.tttt` | String | Direct copy | Debug field |
| `FormattedKINRegisteredPhone` | `next_of_kin.phone_formatted` | String | Direct copy | Formatted KIN phone |
| `Kin_Alternate_Contact_ID` | `next_of_kin.alternate_contact_id` | String | Direct copy | KIN alternate ID |
| `CustomerLocation` | `customer.location.county` | String | Direct copy | Customer location |
| `front_id_photo` | `kyc_documents.front_id_photo` | Array | File URLs | ID front photo |
| `customer_holding_ID` | `kyc_documents.customer_holding_ID` | Array | File URLs | Customer holding ID |
| `six_months_MPESA_statement` | `kyc_documents.six_months_MPESA_statement` | Array | File URLs | MPESA statements |
| `password_MPESA_statement` | `kyc_documents.mpesa_password` | String | Direct copy | MPESA statement password |
| `KRA_pin` | `customer.personal_info.kra_pin` | String | Direct copy | KRA PIN |
| `bank_statement` | `kyc_documents.bank_statement` | Array | File URLs | Bank statements |
| `bank_statement_pin` | `kyc_documents.bank_statement_password` | String | Direct copy | Bank statement password |
| `fraud_checks` | `validation.fraud_checks` | String | Direct copy | Fraud check results |
| `fraud` | `validation.fraud_flag` | Boolean | Parse checkbox | Fraud indicator |
| `customer against phone value (from fraud)` | `validation.customer_phone_fraud` | String | Direct copy | Phone fraud check |
| `customerLocation_for_api` | `customer.location.api_location` | String | Direct copy | API location field |
| `kyc_document_link` | `kyc_documents.link` | String | Direct copy | KYC document link |
| `kyc_document_status` | `kyc_documents.status` | String | Direct copy | KYC document status |
...
| **PostgreSQL Column** | **Default/Derived Value** | **Notes** |
|---|---|---|
| `customer_national_id_number` | Extract from applications | From identification.number |
| `customer_gender` | NULL | Not in current Airtable data |
| `customer_secondary_phone` | NULL | Not in current Airtable data |
| `customer_source` | 'airtable' | Migration source |
| `selfie_url` | Extract from KYC documents | From applications |
| `id_front_url` | Extract from KYC documents | From applications |
| `id_back_url` | Extract from KYC documents | From applications |
| `associated_phone_numbers` | Array of related phones | Aggregate from various sources |
| `customer_updated_at` | NOW() | Migration timestamp |
...
| **PostgreSQL Column** | **Default Value** | **Logic** |
|---|---|---|
| `id` | `gen_random_uuid()` | Primary key |
| `reviewable_type` | `'customer_service'` | Default review type |
| `reviewer_type` | `'customer'` | All reviews from customers |
| `reviewer_name` | Lookup from customer | Customer name |
| `is_verified` | `true` | Phone verified purchase |
| `is_featured` | `false` | Default not featured |
| `status` | `'approved'` | Auto-approve historical |
| `title` | Generate from rating | Auto-generate title |
| `comment` | `NULL` | No comments in source |
...
This collection will be linked to the `applications` collection via:
- `customer_id` → `applications.customer.identification.number`
- `external_lead_reference` → `applications.airtable_record_id`
...
Since this is Transunion data, set:
...
// Helper functions
function parsePhoneNumbers(phoneNumberString: string): string[] {
    if (!phoneNumberString) return [];
    return phoneNumberString.split(',').map(phone => phone.trim()).filter(phone => phone.length > 0);
}
...
Since this is Metropol data, set:
- `providers` = `"METROPOL"`
...
// Helper function to parse Metropol phone numbers
function parseMetropolPhones(phoneNumbersField: string, phonesArray: any[]): string[] {
    const phones = [];
    
    // Parse PhoneNumbers field
    if (phoneNumbersField) {
        try {
            const parsed = JSON.parse(phoneNumbersField);
            if (Array.isArray(parsed)) {
                phones.push(...parsed);
            }
        } catch {
            // If not JSON, treat as single phone
            phones.push(phoneNumbersField);
        }
    }
    
    // Parse phones array
    if (Array.isArray(phonesArray)) {
        phones.push(...phonesArray);
    }
...
The current CSV appears to have minimal data:
...
Since the Airtable data is minimal, we'll need to derive or default these:

| **MongoDB Field** | **Default/Derived Value** | **Notes** |
|---|---|---|
| `_id` | `new ObjectId()` | Generated ID |
| `userId` | Extract from request body | User receiving notification |
| `title` | Extract from request body or generate | Notification title |
| `body` | Extract from request body | Notification content |
| `channel` | Extract from request body or default | Delivery channel |
| `read` | `false` | Default unread |
| `readAt` | `null` | Not read |
| `updatedAt` | `created_at` | Last update |
...
From the application code, we can see these fields are extracted:
...
| **MongoDB Field** | **Derived Value** | **Notes** |
|---|---|---|
| `_id` | `new ObjectId()` | Generated primary key |
| `target_id` | Link to application | From application lookup |
| `target` | `'APPLICATION'` | Target type |
| `document_id` | Link to KYC document | If statement document exists |
| `status` | `'COMPLETED'` | Analysis completion status |
| `type` | `'MPESA'` | Statement type |
| `provider` | `'MPESA'` | Service provider |
| `twelve_months_loan_price` | Calculate | 12-month loan affordability |
| `gap_analysis` | Nested object | Gap analysis results if available |
| `jsonData` | Complete analysis | Full analysis data |
| `raw` | Minified JSON | Compressed raw data |
| `statement_source` | `'UPLOADED'` | How statement was obtained |
...
// Helper to parse any additional fields that might be in the CSV
function parseAdditionalFields(record: any): any {
    const additionalFields = {};
    const knownFields = [
        'Name', 'CustomerId', 'PhoneNumber', 'Email', 'Duration', 'StatementPeriod',
        'AgentDeposit', 'AirtimeHighest', 'InternetBundleHighest', 'PersonsReceivedFromTotal',
        'BanksReceived', 'MFIsReceived', 'MobileLendersReceived', 'HealthcareReceived',
        'SaccosReceived', 'InsuranceReceived', 'BettingReceived', 'OthersReceived',
        'AirtimeTotal', 'InternetBundleTotal', 'TotalPaidIn', 'TotalPaidInAverage',
        'TotalOthers', 'Affordability', 'AverageIncome', 'sourceDB'
    ];
    
    Object.keys(record).forEach(key => {
        if (!knownFields.includes(key) && record[key] !== undefined && record[key] !== '') {
            additionalFields[key] = record[key];
        }
    });
...
This detailed mapping provides the exact field-by-field transformation logic needed to migrate from Airtable to the new PostgreSQL/MongoDB architecture while preserving all data relationships and business logic.